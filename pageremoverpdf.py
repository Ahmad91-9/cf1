...
class PDFEditorMain(QWidget):
    def __init__(self):
        ...
        # store erasures:
        # per-page list of EraseRect (for single-page erases)
        self.per_page_erases: Dict[int, List[EraseRect]] = {}
        # global erases: list of EraseRect with global_rect=True
        self.global_erases: List[EraseRect] = []

        # store modified PIL images for pages when user edits - used at save time (we keep a cache if user needs it)
        # Note: with option 2 (preview overlays), we don't need to immediately create modified images; SaveWorker will render + apply.
